...
その中の一つが [ActiveX](https://ja.wikipedia.org/wiki/ActiveX) の形式で渡されて「こんなん無理や」と言ったら DLL でラップしたものを送ってもらえてなんとかなりました。[^5]
...
提供される DLL に 32bit 版しか用意されていないものがあり、アプリ全体を 32bit にする必要がありました。
64bit 版 Windows で 32bit の Electron アプリを動かす格好です。
今時 32bit って…と思いつつも、現実ってこんなもんなんかなと対応しました。

## 音声再生

ページ遷移した時にナビゲーションの音声を再生させる必要がありますが、Electron のブラウザ上で音声を再生してみると処理性能の問題で遅延が大きくて使い物になりませんでした。
...
ただし、FFI をメインプロセスから実行すると呼び出し先の DLL が応答なしになったりクラッシュした場合はメインプロセスも巻き込まれてしまいます。
...
WebSocket も踏み台サーバーのポートフォワーディングを経由しています。
...
報告があった筐体 PC にリモートデスクトップを行うと驚くほど重く、何分かかけてタスクマネージャーまで辿り着くとディスクの読み書きが 100%に張り付いていました。
...
これもネットワーク障害と似ていますが、USB の接続不良が多いです。

挿し直しても直らない場合もあります。こちらとしてはケーブルの断線かハードウェアの問題だろうと思いますが、筐体の提供企業から「ソフトウェアの問題では」と回されることがあります。
