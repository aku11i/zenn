...
Electron(Node.js) のアプリケーションで、別言語で作成したモジュールを動かす時に `child_process` を使用すると思います。
...
`execFile('mkdir', ["$HOME"])` と実行されても、`$HOME`は展開されずに文字列のまま渡されます。

シンプルな 1 コマンドで完結し、引数がユーザーや外部の要因により動的に変動する場合には `execFile` を使用するのが無難です。
第一引数のコマンドも、 `/bin/mkdir` と絶対パスで指定した方がより安全です。

# まとめ

- exec/execSync はシェルで実行されるため、外部要因に影響する動的なコマンドを実行する場合は必ずサニタイズしなければならない
- クロスプラットフォームである Electron の場合は OS によって使用されるシェルが変わるため、サニタイズは難しい
